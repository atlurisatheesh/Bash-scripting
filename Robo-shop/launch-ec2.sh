...
# SG_ID="$(aws ec2 describe-security-groups  --filters Name=group-name,Values=atluri | jq '.SecurityGroups[].GroupId' | sed -e 's/"//g')"       # b54-allow-all security group id
...
aws ec2 run-instances --image-id ${AMI_ID} --instance-type ${INSTANCE_TYPES} --security-group-ids ${SECURITY_ID} --tag-specifications "ResourceType=instance,Tags=[{Key=name,Value=${COMPONENT}}]"
